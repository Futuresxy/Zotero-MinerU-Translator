...
function createMessages(
  ...
  messages.push({
    ...
    content: [
      ...
      "5. 人名、机构名、数据集名、方法名优先采用学术场景常见译法，不确定时保留原文。",
      "6. 如果当前分段包含图表包装、details/summary、算法伪代码或明显非正文块，不要补写或解释它们，只处理正文内容。",
      "7. 如果当前分段从句子中间开始或结束，只翻译收到的内容，不要自行补全缺失上下文。",
      `8. 这是第 ${index + 1} / ${total} 个分段，请直接返回译文 Markdown。`,
      "",
      chunk,
    ].join("\n"),
  });
